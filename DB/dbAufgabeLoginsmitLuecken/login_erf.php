...
<html>
    ...
    alert("Bitte nutzen Sie ein Passwort mit mindestens 8 Zeichen ein!");
    document.formular.passwort1.focus();
    return false;
    
    var nr_length = document.formular.passwort1.value.replace(/[^0-9]/g, '').length;
    if(nr_length < 1) {
    alert("Das Passwort muss mindestens 1 Zahl haben");
    document.formular.passwort1.focus();
    return false;
    ...
    // Ihr JavaScript-Code --> Passwortlänge ... entfernen Sie bei den nächsten 2 Zeilen die Kommentar-Zeichen (//) und ergänzen Sie korrekt!
    //if(document.formular.passwort1.value.length < 8) {
    //}
...
if (isset($_POST['erfassen']) OR isset($_POST['anpassen']))
{
  ...
  $pass = md5($passwort1);
	...
	// Benutzer erfassen, weil noch nicht in DB vorhanden
        // 
        $insert = "INSERT INTO `users` (`user_id`, `user_name`, `password`, `email`) VALUES ('', '$benutzername', '$passwort', '$email')";
        mysqli_query($link, $insert) or die ("DB-Eintrag hat nicht geklappt");
        //echo "<p> Daten wurden erfasst </p></br>";
